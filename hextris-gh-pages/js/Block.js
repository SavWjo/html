function Block(fallingLane, color, iter, distFromHex, settled) {
	// независимо от того, опирается ли блок на центральный шестиугольник или на другой блок
	this.settled = (settled === undefined) ? 0 : 1;
	this.height = settings.blockHeight;
   //линии, с которого падает блоки
	this.fallingLane = fallingLane;

		this.checked=0;
    //угол падения блока
	this.angle = 90 - (30 + 60 * fallingLane);
	//для расчета вращения блоков, прикрепленных к центральному шестиугольнику
	this.angularVelocity = 0;
	this.targetAngle = this.angle;
	this.color = color;
	//блоки, которые планируется удалить после получения действительного результата
	this.deleted = 0;
	//блоки, которые нужно убрать из падения и добавить в шестиугольник
	this.removed = 0;
	//значение непрозрачности белого блока, нарисованного поверх падающего блока, чтобы придать ему свечение при прикреплении к шестиугольнику
	this.tint = 0;
	//значение, используемое для анимации удаления
	this.opacity = 1;
	//логическое значение, когда блок расширяется
	this.initializing = 1;
	this.ict = MainHex.ct;
	//скорость блока
	this.iter = iter;
	//количество итераций перед началом отбрасывания
	this.initLen = settings.creationDt;
	//сторона, на которой тоже прикреплен блок
	this.attachedLane = 0;
	//расстояние от центра шестиугольника
	...
	this.incrementOpacity = function() {
		if (this.deleted) {
			...
			if (this.opacity <= 0) {
				//планируем окончательное удаление
				this.opacity = 0;
				this.deleted = 2;
	...
	this.getIndex = function (){
		// получаем индекс блока в его стеке
	...
	this.draw = function(attached, index) {
		...
		//ширина = эта ширина + эта высота + 3;
		var p1;
		var p2;
		var p3;
		var p4;
