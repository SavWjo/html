function exportSaveState() { //Сохраняет текущий прогресс
	var state = {};
...
function descaleBlock(b) { //удаляет блоки
	b.distFromHex /= settings.scale;
}

function writeHighScores() { //выводит рекорды
		highscores.sort(
		function(a,b){
			a = parseInt(a, 10);
			b = parseInt(b, 10);
...
function clearSaveState() { //очищает побитые рекорды 
	localStorage.setItem("saveState", "{}");
}

function isStateSaved() { //сохраняет
