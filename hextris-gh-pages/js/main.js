function scaleCanvas() { //масштаб игры
    ...
    setBottomContainer(); //установить нижние кнопки
    set_score_pos(); //установить позицию счета
}
...
function toggleDevTools() { //включить нижнюю панель
	$('#devtools').toggle();
}

function resumeGame() { //продолжить игру
	gameState = 1;
	hideUIElements();
	$('#pauseBtn').show();
	$('#restartBtn').hide();
	importing = 0;
	startTime = Date.now();
	setTimeout(function() { //установить тайм-аут
		if ((gameState == 1 || gameState == 2) && !$('#helpScreen').is(':visible')) {
			$('#openSideBar').fadeOut(150, "linear");
		}
	}, 7000);

	checkVisualElements(0); //проверять визуальные элементы в случае проигрыша
}
...
function hideUIElements() { //скрыть элементы пользовательского интерфейса
	$('#pauseBtn').hide();
	$('#restartBtn').hide();
	$('#startBtn').hide();
}
