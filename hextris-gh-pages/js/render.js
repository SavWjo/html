function render() { //дает изображение
	var grey = '#bdc3c7';
	if (gameState === 0) {
	...
	clearGameBoard(); //очищает игровые советы
...
function renderBeginningText() { //начальный текст с использованием многих устройств
    ...
    if(/mobile|Mobile|iOS|Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        mob = true;
        input_text = 'используйсте экран влево или вправо'
        action_text = 'чтобы вращать шестиугольник'
        score_text = 'Соберите 3+ блока, чтобы получить очки'
        fontSize = 35
    } else {
        mob = false
        input_text = 'Используйте клавиши со стрелками вправо и влево'
        action_text = 'чтобы вращать шестиугольник'
        score_text = 'Соберите 3+ блока, чтобы получить очки'
        fontSize = 27
    }
