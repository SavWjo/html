function drawTimer() {
	if(gameState==1){ //статус игры
		var leftVertexes = [];
		var rightVertexes = [];
	if(MainHex.ct - MainHex.lastCombo < settings.comboTime){
		for(var i=0;i<6;i++){
			var done = (MainHex.ct -MainHex.lastCombo); //Шестигранник следит за последней комбинацией и временем
...
function calcSide(startVertex,endVertex,fraction,offset){ //расчет cторон
...
function drawSide(vertexes){ //вывод сторон
	if (gameState === 0) {
